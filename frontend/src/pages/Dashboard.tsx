  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        // For organization-specific data, we'll use the existing endpoints
        // The backend should filter by user's organization automatically
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-6">
        ...
        <div className="text-sm text-gray-500">
          ...
          {user?.role && <p className="text-indigo-600">Role: {user.role}</p>}
          {user?.organization?.name && <p className="text-green-600">Organization: {user.organization.name}</p>}
        </div>
      </div>
