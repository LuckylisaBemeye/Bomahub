...
const TenantDetails = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    <Link 
                      to={`/properties/${tenancy.unit.property?.id || '#'}`}
                      className="text-indigo-600 hover:text-indigo-900"
                    >
                      {tenancy.unit.property?.name || 'Unknown Property'}
                    </Link>
                  </p>
                  <p className="text-xs text-gray-500 mt-1">
                    {tenancy.unit.property?.address || 'Unknown Address'}
                  </p>
                </div>
      ...
      <div className="bg-white shadow rounded-lg mb-8">
        <div className="px-6 py-5 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">Properties ({new Set(tenantTenancies.map(t => t.unit.property?.id).filter(Boolean)).size})</h2>
        </div>
        <div className="px-6 py-5 space-y-6">
          {/* Group tenancies by property */}
          {Array.from(new Set(tenantTenancies.map(t => t.unit.property?.id).filter(Boolean))).map(propertyId => {
            const propertyTenancies = tenantTenancies.filter(t => t.unit.property?.id === propertyId);
            const property = propertyTenancies[0].unit.property;
