...
const Alert: React.FC<AlertProps> = ({ type, message, onDismiss }) => {
  ...
  return (
    <div className={`p-4 rounded-md ${alertClasses[type]}`}>
      <div className="flex">
        <div className="flex-shrink-0">
          {renderIcon()}
        </div>
        <div className="ml-3 flex-1">
          <p className={`text-sm ${textClasses[type]}`}>{message}</p>
        </div>
        {onDismiss && (
          <div className="ml-auto pl-3">
            <div className="-mx-1.5 -my-1.5">
              <button
                type="button"
                onClick={onDismiss}
                className={`inline-flex rounded-md p-1.5 focus:outline-none ${buttonClasses[type]}`}
              >
